...
select "Н_ЛЮДИ"."ФАМИЛИЯ", "Н_ВЕДОМОСТИ"."ДАТА", "Н_УЧЕНИКИ"."НАЧАЛО" FROM "Н_ВЕДОМОСТИ"
left join "Н_ЛЮДИ" ON "Н_ЛЮДИ"."ИД" = "Н_ВЕДОМОСТИ"."ЧЛВК_ИД"
left join "Н_УЧЕНИКИ" ON "Н_УЧЕНИКИ"."ЧЛВК_ИД" = "Н_ЛЮДИ"."ИД"
WHERE date_part('year', age("Н_ВЕДОМОСТИ"."ДАТА"::date, "Н_УЧЕНИКИ"."НАЧАЛО"::date)) = 1
AND "Н_ВЕДОМОСТИ"."ОЦЕНКА" = 'незач'
and lower(substring("Н_ЛЮДИ"."ФАМИЛИЯ" FROM 1 FOR 1)) IN ('б', 'в', 'г')

GROUP BY "Н_ВЕДОМОСТИ"."ЧЛВК_ИД", "Н_ЛЮДИ"."ФАМИЛИЯ", "Н_УЧЕНИКИ"."НАЧАЛО", "Н_ВЕДОМОСТИ"."ДАТА"
having count(*)>=2
